...
class User_api extends Base_Controller 
{
					...
					if($old_password!= $new_password)
					{
							...
							if($old_password==$result->password)
							{
								if($new_password == $confirm_password)
								{
									...
									return array('state'=>True,'msg'=>'Password Changed Succesfully!','details'=>$result1);
								}
								else
								{
							...
							{
								return array('state'=>False,'msg'=>'Current Password Not Match!','details'=>False);
							}	
					}
					else
					{
						return array('state'=>False,'msg'=>'Current and New Password are Same!','details'=>False);
					}		
				}
				else
				{
					return array('state'=>False,'msg'=>'Current Required!','details'=>False);
				}
			}
			else
			{
	...
	*/
	public function _post_activation_link($details=array())
    {
		...
		{ 
			$userdata = $this->common_model->selectDetailsWhr('tbl_user','email',$email_id);
			if($userdata)
			{
				...
				if($user)
				{
					$datavalue["email"]=$email_id;
					$datavalue["fullname"]=$user1;
					$datavalue["activation_code"]=$activation_code;
					$subject = 'Vipassana Activation Code';
					$msg_body  = $this->load->view('mail/recovery-link',$datavalue,true);
					$alt_msg = 'Vipassana Activation Code';                  
					$data1=array('subject'=>$subject,'msg_body'=>$msg_body,'alt_msg'=>$alt_msg);             
					$email_id1[]=array('email_id'=>$email_id); 
					$message =$this->load->view('mail/recovery-link',$datavalue,true);
					$mail_result=$this->email_sent->mail_sent_by_webmail($data1,$email_id1); 
					unset($email_id); 
					return array(
							'state'=>True,
		...
		if(isset($activation_code) && !empty($activation_code))
		{
		    ...
		    if($fetch_activation_code)
			{
				if($fetch_activation_code->activation_code == $activation_code)
				{
					...
					//$rest_user_id=$this->session->userdata('activation_code');
					if($user_id)//2
				    {
						$data1=array(
							'user_id'=>$user_id,
						...
						{
							return array('state'=>True,'msg'=>'Verify Activation Code!','details'=>$user_id,
				     ...
				     {
					 	return array('state'=>False,'msg'=>'No Account this Mail !','details'=>False,
					 	'redirect' => base_url().'forgot-password');
			    	}
				}
				else
				{
					return array('state'=>False,'msg'=>'Activation Code not Match!','details'=>False,
					'redirect' => base_url().'forgot-password');
				}
			}
			else
	...
	*/
	public function _post_reset_password($details=array())
    {
		...
		$fetch_user=$this->common_model->selectDetailsWhr('tbl_user','user_id',$user_id);
