...
class Excel_writer
{
	...
	function createExcel($title,$description,$inputdata,$filename,$mode='O',$each=false)
	{
		...
		{		
			...
			if($each)
			{
				$this->fillSheetComplex($newsheet,$sheet_data );
			}
			else
			{
	...
	function fillSheetComplex(&$sheet,$sheet_data)
	{
		if(isset($sheet_data['data']) && is_array($sheet_data['data']))
		{
			foreach ($sheet_data['data'] as $records)
			{
				$style=array();
				...
				$headinh_range= $this->fillCellArray($sheet,$records['headings']);
				$Start=($sheet->getHighestRow()==1)?0:$sheet->getHighestRow();
				$headingHeight=count($records['headings']);
				$max_range=$sheet->getHighestColumn();
				$headinh_range.=':'.$max_range.$headingHeight;
				$this->applyStyle($sheet,$style,$headinh_range);
			}
		}
	}
	function fillCell(&$sheet,$dataRow)
	{
	...
	function fillCellArray(&$sheet,$dataRow)
	{
		...
		if(is_array($dataRow)&& count($dataRow)>0)
		{
			...
			$ploat['start']=$letters[$col].$row;
			$sheet->fromArray($dataRow, NULL, $ploat['start']);
			return $letters[$col].$row;
		}
		
	}
	function higestRow(&$sheet)
	{
		$Start=($sheet->getHighestRow()==1)?0:$sheet->getHighestRow();
			$col=0;
			if($Start==0)
